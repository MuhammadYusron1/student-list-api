...
export default function EditStudentForm({active, onActiveChange}){
				...
				<div className='relative flex flex-col w-full bg-white border border-solid rounded pointer-events-auto border-black/20 bg-clip-padding outline-0'>
					<form>
						...
						<div className='flex items-start justify-between px-8 py-5 border-b border-solid border-[#dee2e6]'>
							...
							<button onClick={() => onActiveChange({...active, edit: !active.editClose})} className='px-4 py-4 ml-auto -my-4 -mr-4'><i className="fa-solid fa-xmark fa-lg"></i></button>
						</div>
						{/* Modal Body */}
						...
						<div className='flex px-8 py-5 bg-[#ecf0f1] flex-wrap items-center justify-end border-t border-solid border-[#dee2e6] '>	
							<button onClick={() => onActiveChange({...active, edit: !active.editClose})} className="m-1 text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">Cancel</button>
							<button className="m-1 text-white bg-[#17a2b8] hover:bg-[#4bb8c9] focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 capitalize">save</button>
						</div>
					</form>
				</div>
			</div>
